(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e){if(t.type!==`childList`)continue;for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();const e=`modulepreload`,t=function(e){return`/front_7th_chapter2-1/`+e},n={},r=function(r,i,a){let o=Promise.resolve();if(i&&i.length>0){let r=function(e){return Promise.all(e.map(e=>Promise.resolve(e).then(e=>({status:`fulfilled`,value:e}),e=>({status:`rejected`,reason:e}))))},s=document.getElementsByTagName(`link`),c=document.querySelector(`meta[property=csp-nonce]`),l=c?.nonce||c?.getAttribute(`nonce`);o=r(i.map(r=>{if(r=t(r,a),r in n)return;n[r]=!0;let i=r.endsWith(`.css`),o=i?`[rel="stylesheet"]`:``,c=!!a;if(c)for(let e=s.length-1;e>=0;e--){let t=s[e];if(t.href===r&&(!i||t.rel===`stylesheet`))return}else if(document.querySelector(`link[href="${r}"]${o}`))return;let u=document.createElement(`link`);if(u.rel=i?`stylesheet`:e,i||(u.as=`script`),u.crossOrigin=``,u.href=r,l&&u.setAttribute(`nonce`,l),document.head.appendChild(u),i)return new Promise((e,t)=>{u.addEventListener(`load`,e),u.addEventListener(`error`,()=>t(Error(`Unable to preload CSS for ${r}`)))})}))}function s(e){let t=new Event(`vite:preloadError`,{cancelable:!0});if(t.payload=e,window.dispatchEvent(t),!t.defaultPrevented)throw e}return o.then(e=>{for(let t of e||[]){if(t.status!==`rejected`)continue;s(t.reason)}return r().catch(s)})},i=()=>`
    ...
    `,a=()=>`<footer class="bg-white shadow-sm sticky top-0 z-40">
      <div class="max-w-md mx-auto py-8 text-center text-gray-500">
        <p>© 2025 항해플러스 프론트엔드 쇼핑몰</p>
      </div>
    </footer>`,o=`
                  <div class="text-sm text-gray-500 italic">카테고리 로딩 중...</div>
`,s=e=>{console.log(e);let t=Object.keys(e);return`
    ${t.map(e=>`
      <button data-category1="${e}" data-category2="생활용품" class="category2-filter-btn text-left px-3 py-2 text-sm rounded-md border transition-colors bg-white border-gray-300 text-gray-700 hover:bg-gray-50">
        ${e}
      </button>
    `).join(``)}
`},c=e=>{let t=e?.sort||`price_asc`,n=[{value:`price_asc`,label:`가격 낮은순`},{value:`price_desc`,label:`가격 높은순`},{value:`name_asc`,label:`이름순`},{value:`name_desc`,label:`이름 역순`}];return`
                  <select id="sort-select" class="text-sm border border-gray-300 rounded px-2 py-1
                               focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                    ${n.map(e=>`<option value="${e.value}" ${t==e.value?`selected`:``}>${e.label}</option>`).join(``)}
                  </select>
`},l=e=>{let t=e?.limit||20,n=[10,20,50,100];return`
...
`},u=({filters:e,pagination:t,loading:n,categories:r})=>(console.log(e),`
...
`,f=`<div class="text-center py-4">
              ...
              </div>`,p=({title:e,lprice:t,image:n,productId:r})=>`
    ...
    `,m=({loading:e,products:t,pagination:n})=>(console.log(`pagination`,n),`
  <div class="mb-6">
            <div>
            ${e?`
                <!-- 상품 그리드 -->
...
${d.repeat(4)}
              ...
              ${f}`:`<div class="mb-4 text-sm text-gray-600">
            ...
            <div>
              ...
              <div id="products-grid">
                <div class="flex flex-col justify-center items-center">
                  ...
                  <button class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600" onclick="window.location.reload()">다시 시도</button>
                </div>
              </div>
            </div>
          </div>
      </div>
      `}),S=()=>r(async()=>{let{worker:e}=await import(`./browser-CcyfQrG1.js`);return{worker:e}},[]).then(({worker:e})=>e.start({onUnhandledRequest:`bypass`,serviceWorker:{url:`/front_7th_chapter2-1/mockServiceWorker.js`}})),C=e=>{history.pushState(null,null,e),w()},w=async()=>{let e=document.querySelector(`#root`),t=`/front_7th_chapter2-1/`,n=window.location.pathname,r=n.replace(t,`/`).replace(/\/$/,``)||`/`;if(r===`/`)try{e.innerHTML=g({loading:!0});let t=await _(),n=await y();console.log(t),e.innerHTML=g({loading:!1,...t,categories:n}),document.body.addEventListener(`click`,e=>{if(e.target.closest(`.product-card`)){let t=e.target.closest(`.product-card`).dataset.productId;C(`/front_7th_chapter2-1/product/${t}`),w()}})}catch{e.innerHTML=x()}else{let t=location.pathname.split(`/`).pop();e.innerHTML=b({loading:!0});let n=await v(t);e.innerHTML=b({loading:!1,product:n})}};window.addEventListener(`popstate`,w),document.body.addEventListener(`change`,async e=>{console.log(e.target.value);let t=e.target.value,n=document.querySelector(`#root`);if(e.target.id!==`limit-select`)return;let r={limit:t};n.innerHTML=g({loading:!0});let i=await y(),a=await _(r);console.log(a),n.innerHTML=g({loading:!1,...a,categories:i})}),document.body.addEventListener(`change`,async e=>{if(e.target.id!==`sort-select`)return;let t=e.target.value,n=document.querySelector(`#root`),r={sort:t};n.innerHTML=g({loading:!0});let i=await y(),a=await _(r);console.log(a),n.innerHTML=g({loading:!1,...a,categories:i})});const T=()=>{w()};S().then(T);